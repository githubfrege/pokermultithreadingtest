...
namespace PokerBotApplication
{
    ...
    public static class BotOperations
    {
        ...
        private static float getRiskOfRuin(float odds, int i, int bankRoll)
        {
            ...
            return i == 0 ?  riskOfRuin(bankRoll / 1) : riskOfRuin(bankRoll / i);
        ...
        private static (int heroPoints, int villainPoints) updateScore(ulong holes, ulong villainHoles, ulong currentHand)
        {
        ...
        {
             int heroWins = 0; //if player wins
             int villainWins = 0; //if opponent wins
            ...
            Parallel.ForEach(HandEvaluation.CardCombos(availableCards.ToIEnum(), 2), villainHoles => //every combination of cards our opponent may have
            {
                ...
                if (tableKvpList.Count < 5)
                {
                    foreach (ulong cardAdditions in HandEvaluation.CardCombos(currentAvailableCards.ToIEnum(), 5 - tableKvpList.Count)) //all combinations of cards that may be added to the existing table
                    {
                        ...
                        (int heroPoints, int villainPoints) scoreUpdate = updateScore(holes, villainHoles, currentHand);
                        Interlocked.Add(ref heroWins, scoreUpdate.heroPoints);
                        Interlocked.Add(ref villainWins, scoreUpdate.villainPoints);
                ...
                else
                {
                    ...
                    (int heroPoints, int villainPoints) scoreUpdate = updateScore(holes, villainHoles, communityCards);
                    Interlocked.Add(ref heroWins,scoreUpdate.heroPoints);
                    Interlocked.Add(ref villainWins,scoreUpdate.villainPoints);
                    //Debug.WriteLine($"hero:{heroWins} - villain:{villainWins}");
                    //heroWins += scoreUpdate.heroPoints;
            ...
            Console.WriteLine(heroWins / (heroWins + villainWins));
